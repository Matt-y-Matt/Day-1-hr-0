...
export async function loadExport(userId,start,end){
 const definitions=[['runs','runs','*'],['sessions','sessions','*, workout_days(name,is_daily)'],['pain','pain_logs','*'],['daily','daily_log','*'],['meals','meal_logs','*, foods(name)'],['changes','plan_changes','*'],['photos','photos','id,date,slot'],['workoutChanges','workout_changes','*']];
