...
export function exportMarkdown(data,start,end){
 const {runs=[],sessions=[],logs=[],pain=[],daily=[],meals=[],changes=[],photos=[],workoutChanges=[]}=data;
 ...
 out+=rows('Workout changes',['Date','Scope','Workout','Changes'],workoutChanges.map(c=>[c.date,c.scope,c.workout_day_id,(c.after_items||[]).map(x=>`${x.exercises?.name||x.exercise_id}: ${x.sets} sets${x.is_enabled?'':' · disabled: '+x.disabled_reason}`).join('; ')]));
