...
import {workoutPayload,swapWorkoutExercise} from './workout.mjs';
...
test('swapping a disabled alternative preserves unique slots and clears both loads',()=>{
 const a={id:'a',exercise_id:'e',exercises:{id:'e',name:'RDL',category:'legs'},sets:3,rep_min:8,rep_max:10,rest_seconds:90,is_enabled:true,target_weight_kg:40};
 const b={...a,id:'b',exercise_id:'f',exercises:{id:'f',name:'Curl',category:'legs'},is_enabled:false,disabled_reason:'Alternative'};
 const swapped=swapWorkoutExercise([a,b],'a',b.exercises);
 assert.deepEqual(swapped.map(x=>x.exercise_id),['f','e']);assert.equal(swapped[1].is_enabled,false);assert.equal(swapped[1].disabled_reason,'Swapped out for Curl');assert.ok(swapped.every(x=>x.target_weight_kg===null));assert.equal(workoutPayload(swapped).length,2);assert.equal(a.exercise_id,'e');
 assert.throws(()=>swapWorkoutExercise([a,{...b,is_enabled:true}],'a',b.exercises));assert.throws(()=>swapWorkoutExercise([a,b],'a',{...b.exercises,category:'push'}));
});
