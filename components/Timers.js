...
export function GymRestTimer({seconds,storageKey,nextSet,onDone,beepEnabled=false}) {
 ...
 return <><TimerRing remainingMs={timer.remainingMs} left={timer.left} total={seconds}/><p className="muted">{timer.status==='done'?'Rest complete. Start when you are ready.':'Breathe. Set up for the next set.'}</p><button className="btn" onClick={onDone}>Start set {nextSet} now</button><button className="btn ghost" onClick={timer.extend}>+30 seconds</button></>;
}
