...
function PrivatePhoto({photo}){
 ...
 return error?<div className="flag" role="alert">Photo unavailable<button className="btn ghost" onClick={()=>setRetry(x=>x+1)}>Retry photo</button></div>:url?<PhotoViewer onError={()=>setError('Photo unavailable')} className="compare-photo" src={url} alt={`${photo.date} ${photo.pose||'progress'} photo`}/>:<div className="photo-placeholder">Loading photo…</div>;
}
