...
export default function Progress({userId,revision,subtabs,onExport,onAcwr}){
 ...
 {['running','cycling'].map(sport=>{const a=activityLoad(data.runs,sport,'time',today());return <section className="card" key={sport}><div className="row"><strong>{sport==='running'?'Running':'Cycling'} time · ACWR</strong><b>{a.ratio==null?'—':a.ratio.toFixed(2)}</b></div><p>Last 7 days {a.acute.toFixed(1)} · 28-day weekly average {a.chronic.toFixed(1)}</p><small>Recorded duration · {a.missing} activities missing duration.</small>{onAcwr&&<button className="btn ghost" onClick={onAcwr}>Show formula and sessions →</button>}</section>;})}
