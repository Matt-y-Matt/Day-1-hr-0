...
function DailyRest({ rest, userId, beepEnabled, onDone }) {
  ...
  return <><h1>Rest</h1><TimerRing left={timer.left} total={rest.seconds} label={timer.status === 'done' ? 'READY' : 'REST'} /><p className="muted">Next set starts when you are ready.</p><button className="btn" onClick={onDone}>{timer.status === 'done' ? 'Continue daily block' : 'Skip rest'}</button></>;
}
...
function DailyItem({ item, setNumber, userId, identity, beepEnabled, busy, onBack, onSave }) {
  ...
  return <><p className="u-label">Set {setNumber} of {item.sets}</p><h1>{item.exercises.name}</h1><p className="muted">{item.exercises.cue_execution}</p>
    {hold ? <><TimerRing left={timer.status === 'idle' ? hold : timer.left} total={hold} label={timer.status === 'done' ? 'HOLD FINISHED' : timer.status === 'paused' ? 'PAUSED' : 'HOLD'} />
      ...
      <p className="muted">Log only the hold you completed. The next hold starts when you are ready.</p><button className="btn" disabled={busy || timer.status !== 'done'} onClick={() => onSave(null)}>{busy ? 'Saving…' : `✓ Log completed hold ${setNumber}`}</button><button className="btn ghost" disabled={busy} onClick={timer.reset}>Reset hold</button>
